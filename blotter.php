															...
															<?php if (isset($_SESSION["username"])): ?>
																...
																<td>
																	...
																	<a
																		...
																		data-feedback="<?= ucwords($row["feedback"]) ?>"
																	>
																		<?php if (isset($_SESSION["username"])): ?>
...
<script>
function editBlotter1(element) {
    // Retrieve data attributes from clicked element
    const id = element.getAttribute("data-id");
    const complainant = element.getAttribute("data-complainant");
    const respondent = element.getAttribute("data-respondent");
    const victim = element.getAttribute("data-victim");
    const type = element.getAttribute("data-type");
    const location = element.getAttribute("data-l");
    ...
    document.getElementById("complainant_id").value = id;
    ...
    document.getElementById("feedback").value = feedback || ''; // Default to empty if null
    document.getElementById("blotter_id").value = id;
    ...
    if (!typeMatched) {
        // If the type isn't a listed option, assume it's "Others"
        typeSelect.value = "Others";
        otherInputGroup.style.display = "block";
        otherInput.value = type;
    }

    // Handle Status select (only if it exists)
...
if (statusSelect) {
    ...
    // If there's no select, update the hidden status input
