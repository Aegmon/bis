...
<<<<<<< HEAD
  const identifiedSelect = document.querySelector('.identity');
    ...
    function handleIdentityChange() {
      if (identifiedSelect.value === 'Unregister') {
        voterStatusSelect.value = ""; // Set to null
        voterStatusSelect.setAttribute("readonly", true);
        voterStatusSelect.disabled = true;
      } else {
        voterStatusSelect.disabled = false;
        voterStatusSelect.removeAttribute("readonly");
      }
    }

    identifiedSelect.addEventListener("change", handleIdentityChange);
